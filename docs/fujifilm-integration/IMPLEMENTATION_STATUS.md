...
| 8 — performance, memory, capability gating | PARTIAL — device evidence collected | USB evidence from Moto Edge 60 Fusion: ARM Mali-G615 MC2, OpenGL ES 3.2, 5,532 rendered frames, 203 janky frames (3.67%), 50th-percentile frame 14 ms, 90th-percentile frame 93 ms, 50th-percentile GPU frame 3 ms. A separate `dumpsys meminfo` sample recorded 287,806 KB total PSS, 68,980 KB graphics PSS, and 50,228 KB native heap. These are real device observations, not a formal sustained benchmark; the 90th-percentile GPU value was contaminated by idle/lifecycle samples and is not used as a product claim. No optional LUT path was added, so no LUT capability fallback is claimed. |
