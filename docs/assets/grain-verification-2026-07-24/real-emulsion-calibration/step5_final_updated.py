...
def shipped_grain(c, preset, mono_mix, base, bias, clump, seed=42.0, u_grain=0.0,
    ...
    if eff > 0.001:
        ...
        dw = np.maximum(dw, 0.62 * shadow_floor_scale * (t * t * (3 - 2 * t)))
...
STOCKS = [
    ("Rollei",   0, 0.0, 0.19, 1.00, 1.00, 1.0),   # 2026-07-24: base 0.10->0.19 (RMS 11 vs TriX 17/HIE 18)
    ("HIE",      1, 0.0, 0.24, 1.20, 1.25, 1.0),
    ("FineGrain",4, 0.0, 0.03, 0.65, 0.55, 1.0),
    ("Ektar",    6, 0.0, 0.05, 0.60, 0.45, 1.0),   # 2026-07-24: base 0.02->0.05 (PGI threshold-crossing)
    ("CineStill",7, 0.0, 0.14, 1.00, 1.05, 0.35),  # 2026-07-24: shadowFloorScale 1.0->0.35 (Vision3 DLT)
    ("TriX",     8, 1.0, 0.26, 1.15, 1.35, 1.0),
    ("AeroDense",9, 0.0, 0.10, 1.10, 1.10, 1.0),
]
...
def q3_shadow_readability():
    ...
    for name, preset, mm, base, bias, clump, sfs in STOCKS:
        ...
        for tone in (0.03, 0.08, 0.15):
            ...
            wg = q8(shipped_grain(patch, preset, mm, base, bias, clump, shadow_floor_scale=sfs))
            ng = q8(shipped_grain(patch, preset, mm, 0.0, bias, clump, shadow_floor_scale=sfs))
...
def proof_strips():
    ...
    for name, preset, mm, base, bias, clump, sfs in STOCKS:
        ...
        for t in tones:
            ...
            cells.append(q8(shipped_grain(patch, preset, mm, base, bias, clump, shadow_floor_scale=sfs)))
